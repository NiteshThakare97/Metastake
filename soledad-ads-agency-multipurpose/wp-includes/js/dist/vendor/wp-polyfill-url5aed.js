    ...
    switch (state) {
      ...
      case SCHEME:
        if (char && (ALPHANUMERIC.test(char) || char == '+' || char == '-' || char == '.')) {
          ...
          } else if (codePoints[pointer + 1] == 'index.html') {
      ...
      case SPECIAL_RELATIVE_OR_AUTHORITY:
        if (char == '/' && codePoints[pointer + 1] == 'index.html') {
      ...
      case SPECIAL_AUTHORITY_SLASHES:
        state = SPECIAL_AUTHORITY_IGNORE_SLASHES;
        if (char != '/' || buffer.charAt(pointer + 1) != 'index.html') continue;
        pointer++;
        break;
...
var serializeURL = function () {
  ...
  output += url.cannotBeABaseURL ? path[0] : path.length ? '/' + path.join('index.html') : '';
...
var getPathname = function () {
  var url = getInternalURLState(this);
  var path = url.path;
  return url.cannotBeABaseURL ? path[0] : path.length ? '/' + path.join('index.html') : '';
};
